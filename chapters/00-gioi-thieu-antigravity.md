...
### Bước 1: Tải & Cài Đặt Visual Studio Code (VS Code)

1. Mở trình duyệt, truy cập: **<https://code.visualstudio.com/>**
2. Bấm nút xanh to **"Download for Windows"** (hoặc **"Download for Mac"** nếu dùng MacBook).
3. Mở file vừa tải, bấm **Next → Next → Install → Finish** (giống cài Zalo).

> 💡 **Ghi nhớ:** VS Code hoàn toàn **MIỄN PHÍ** và hợp pháp. Đây không phải phần mềm crack.

### Bước 2: Cài Extension Gemini Code Assist

1. Mở VS Code lên.
2. Bấm vào biểu tượng **4 ô vuông** ở thanh bên trái (Extensions — hoặc gõ `Ctrl+Shift+X`).
3. Trong ô tìm kiếm, gõ: **`Gemini Code Assist`**
4. Bấm nút **"Install"** ở kết quả đầu tiên (của Google).
5. Đợi vài giây — Extension tự cài xong.

### Bước 3: Đăng Nhập Tài Khoản Google

1. Sau khi cài Extension, ở bên phải sẽ xuất hiện biểu tượng **Gemini** (hình ngôi sao).
2. Bấm vào → Chọn **"Sign in with Google"**.
3. Trình duyệt bật lên → Đăng nhập tài khoản Google (như đăng nhập Gmail).
4. Quay lại VS Code → Panel Chat đã sẵn sàng!

> 🔐 **Về API Key:** Nếu bạn dùng gói Advanced, cần thiết lập API Key riêng. Xem chi tiết ở [Chương 1B — So Sánh Model](01b-mo-hinh-ai-quyen-luc.md) và [Phụ Lục Cài Đặt Môi Trường](phu-luc-cai-dat-moi-truong.md).

### Bước 4: Mở Thư Mục Dự Án & Bắt Đầu Chat

1. Trong VS Code, bấm **File → Open Folder** (Mở Thư mục).
2. Chọn thư mục chứa dữ liệu công ty (ví dụ: `D:/Du_An_ABC/` hoặc `Desktop/Data_KeToan/`).
3. Mở Panel Chat bên phải (biểu tượng Gemini).
4. Gõ câu lệnh đầu tiên: *"Liệt kê toàn bộ file trong thư mục này cho tôi"*.

🎉 **Chúc mừng! Bạn đã chính thức sở hữu một "Nhân viên Số" sống ngay trong laptop!**
...
| AI trả kết quả sai hoặc "ảo giác" (Hallucination) | Prompt chưa đủ ràng buộc, AI tự suy diễn số liệu. | Thêm Hàng Rào Tử Hình: *"Cấm đoán mọi hành vi đoán mò (Guess). Nếu cột trống, ghi rõ NULL."* Xem chi tiết ở [Chương 8: Bảo Mật](08-bao-mat.md). |
...
| 2 | **Opus không nuốt được file lớn** | Ném file Excel 50MB cho Opus → lỗi tràn bộ nhớ. | File lớn phải dùng **Gemini 3.1 Pro** (Context Window 1-2 triệu Token). Xem [Chương 1B](01b-mo-hinh-ai-quyen-luc.md). |
...
| 5 | **Ảo giác (Hallucination)** | AI bịa số liệu khi dữ liệu không đủ hoặc Prompt thiếu ràng buộc. | Luôn bật "Hàng Rào Tử Hình" trong Prompt: *"Cấm đoán tuyệt đối việc suy diễn/đoán mò"*. Xem [Chương 8](08-bao-mat.md). |
| 6 | **Phụ thuộc Quota API** | Dùng gói Free thì giới hạn 15-50 request/ngày, rất nhanh hết. | Dùng gói Vertex AI Pay-as-you-go hoặc Anthropic API. Xem bảng Quota ở [Chương 1B](01b-mo-hinh-ai-quyen-luc.md). |
...
1. **Xây dựng Kho Tri Thức Tổ Chức (Knowledge Management):** Mỗi quy trình nghiệp vụ được đóng gói thành Skill `.md` → nhân sự mới vào công ty chỉ cần gõ lệnh, không cần ai đào tạo. *(Xem chi tiết: [Chương 7 — Skills & Workflows](07-skills-va-workflows.md))*
...
5. **Ra Quyết Định Dựa Trên Data (Data-Driven Decision):** Thay vì tranh cãi bằng cảm tính, CEO ném câu hỏi chiến lược cho AI mô phỏng kịch bản Monte Carlo. *(Xem: [Chương 6 — Ra Quyết Định](06-business-decision-making.md), [Skill Phân Tích Quyết Định](../skills/phan_tich_quyet_dinh/SKILL.md))*
...
- [Chương 1B — So sánh Gemini vs Opus](01b-mo-hinh-ai-quyen-luc.md)
